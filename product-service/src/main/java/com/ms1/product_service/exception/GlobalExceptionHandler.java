...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDuplicateSku(DuplicateSkuException exception, HttpServletRequest request) {
        log.warn("event=handled_exception type=DuplicateSkuException path={} message={}", request.getRequestURI(), exception.getMessage());
        return buildResponse(HttpStatus.CONFLICT, "DUPLICATE_SKU", exception.getMessage(), request.getRequestURI(), List.of());
    }
